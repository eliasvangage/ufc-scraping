...
const StatCard = ({
  ...
  <div
    className={cn(
      ...
      color === "blue" &&
        "from-blue-500/15 to-blue-500/5 border-blue-500/30 hover:shadow-blue-500/20",
      color === "green" &&
        "from-green-500/15 to-green-500/5 border-green-500/30 hover:shadow-green-500/20",
      color === "purple" &&
        "from-purple-500/15 to-purple-500/5 border-purple-500/30 hover:shadow-purple-500/20",
      color === "red" &&
        "from-red-500/15 to-red-500/5 border-red-500/30 hover:shadow-red-500/20",
      color === "orange" &&
        "from-orange-500/15 to-orange-500/5 border-orange-500/30 hover:shadow-orange-500/20",
      color === "primary" &&
        "from-primary/15 to-primary/5 border-primary/30 hover:shadow-primary/20",
    )}
  >
  ...
  return (
    <div
      className={cn(
        ...
        isWin &&
          "bg-gradient-to-br from-green-500/10 to-emerald-500/5 border-green-500/20 hover:shadow-green-500/20",
        !isWin &&
          !isDraw &&
          "bg-gradient-to-br from-red-500/10 to-rose-500/5 border-red-500/20 hover:shadow-red-500/20",
        isDraw &&
          "bg-gradient-to-br from-yellow-500/10 to-amber-500/5 border-yellow-500/20 hover:shadow-yellow-500/20",
      )}
      style={{
...
export default function FighterProfile() {
      ...
      try {
        ...
        if (isMounted && fighterData) {
          ...
          const transformedFighter: Fighter = {
            ...
            fight_history:
              fighterData.fight_history?.map((fight) => ({
        ...
        console.warn(
          "API fighter data unavailable, using fallback data for:",
          fighterName,
          err,
        );
        ...
        <div className="text-center space-y-6">
          ...
          <div className="space-y-2">
            ...
            <p className="text-muted-foreground">
              Fetching detailed fighter information and statistics
            </p>
          </div>
        </div>
      </div>
  ...
  const recentWins =
    fighter.fight_history?.slice(0, 10).filter((f) => f.result === "win")
      .length || 0;
  const finishRate =
    fighter.fight_history?.filter(
      (f) => f.method && !f.method.toLowerCase().includes("decision"),
    ).length || 0;
  const finishPercentage = fighter.fight_history?.length
    ? Math.round((finishRate / fighter.fight_history.length) * 100)
    : 0;
        ...
        <div className="container mx-auto px-4 py-16">
          <div className="text-center space-y-8 animate-fadeInUpOnce">
            ...
            <div className="flex items-center justify-center gap-4 flex-wrap">
              ...
              <Badge
                variant="outline"
                className="border-muted-foreground/30 text-muted-foreground px-4 py-2"
              >
      ...
      <div className="container mx-auto px-4 py-8 space-y-12">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="space-y-1">
                ...
                <p className="text-sm text-muted-foreground">
                  Fighter data not available from live database. Showing demo
                  information for preview purposes.
                </p>
              </div>
            </div>
          </div>
            ...
            <CardContent className="space-y-6">
              <div className="text-center space-y-4">
                <Badge
                  variant="outline"
                  className={cn(
                    getRecordBadgeColor(fighter.record),
            ...
            <CardContent className="space-y-6">
              <div className="space-y-4">
                <div className="bg-background/30 rounded-lg p-4 border border-purple-500/20">
                  ...
                  <p className="text-sm text-muted-foreground">
                    {fighter.stance} stance fighter with{" "}
                    {winPercentage >= 70
                      ? "excellent"
                      : winPercentage >= 50
                        ? "solid"
                    ...
                    record. Shows{" "}
                    {finishPercentage >= 50
                      ? "aggressive finishing ability"
                      : "technical decision-focused approach"}
                    .
                  </p>
                </div>
                ...
                <div className="bg-background/30 rounded-lg p-4 border border-purple-500/20">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="font-medium text-purple-400">
                      Experience & Competition Level
                    </span>
                  </div>
                  <p className="text-sm text-muted-foreground">
                    {(fighter.fight_history?.length || 0) >= 15
                      ? "Seasoned veteran"
                      : (fighter.fight_history?.length || 0) >= 8
                        ? "Experienced fighter"
                        : "Rising prospect"}{" "}
                    with {fighter.fight_history?.length || 0} professional
                    fights. UFC record of {fighter.ufc_record} demonstrates{" "}
                    {ufcWinPercentage >= 70
                      ? "elite"
                      : ufcWinPercentage >= 50
                        ? "competitive"
                ...
                <div className="bg-background/30 rounded-lg p-4 border border-purple-500/20">
                  ...
                  <p className="text-sm text-muted-foreground">
                    {finishPercentage}% finish rate indicates{" "}
                    {finishPercentage >= 60
                      ? "excellent finishing instincts"
                      : finishPercentage >= 40
                        ? "solid finishing ability"
                        : "decision-oriented fighting style"}
                    . Prefers{" "}
                    {finishPercentage >= 50
                      ? "to end fights early"
                      : "to control pace and win on points"}
                    .
                  </p>
                </div>
                ...
                {fighter.is_champion && (
                  <div className="bg-gradient-to-r from-yellow-500/20 to-yellow-500/10 border border-yellow-500/30 rounded-lg p-4">
                    ...
                    <p className="text-sm text-muted-foreground">
                      Current {fighter.division} Division Champion, representing
                      the pinnacle of skill in the weight class.
                    </p>
                  </div>
                )}
            ...
            {[
              {
                ...
                value: fighter.stats["SLpM"],
                icon: Zap,
                color: "blue",
              },
              {
                label: "Striking Accuracy",
                value: fighter.stats["Str. Acc."],
                icon: Target,
                color: "green",
                progress: parseFloat(
                  fighter.stats["Str. Acc."]?.replace("%", "") || "0",
                ),
              },
              {
                label: "Strikes Absorbed/Min",
                value: fighter.stats["SApM"],
                icon: Shield,
                color: "red",
              },
              {
                label: "Striking Defense",
                value: fighter.stats["Str. Def"],
                icon: Shield,
                color: "purple",
                progress: parseFloat(
                  fighter.stats["Str. Def"]?.replace("%", "") || "0",
                ),
              },
              {
                label: "Takedown Average",
                value: fighter.stats["TD Avg."],
                icon: Target,
                color: "orange",
              },
              {
                label: "Takedown Accuracy",
                value: fighter.stats["TD Acc."],
                icon: Target,
                color: "green",
                progress: parseFloat(
                  fighter.stats["TD Acc."]?.replace("%", "") || "0",
                ),
              },
              {
                label: "Takedown Defense",
                value: fighter.stats["TD Def."],
                icon: Shield,
                color: "blue",
                progress: parseFloat(
                  fighter.stats["TD Def."]?.replace("%", "") || "0",
                ),
              },
              {
                label: "Submission Average",
                value: fighter.stats["Sub. Avg."],
                icon: Activity,
                color: "red",
              },
            ].map((stat, index) => (
        ...
        <section className="space-y-6">
          <div className="text-center space-y-4">
            ...
            <p className="text-muted-foreground">
              Detailed breakdown of all professional fights with performance
              metrics
            </p>
          </div>
          ...
          <Card className="bg-gradient-to-br from-card to-muted/20 shadow-xl">
            <CardHeader>
              <CardTitle className="flex items-center gap-3">
                ...
                Professional Record ({fighter.fight_history?.length || 0}{" "}
                fights)
              </CardTitle>
            </CardHeader>
            <CardContent>
              {fighter.fight_history && fighter.fight_history.length > 0 ? (
                ...
                <div className="text-center py-12 space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-muted-foreground">
                      Fighter data not yet available in our database
                    </p>
                  </div>
                </div>
              )}
