...
export default function ExplainPanel({ 
  ...
  const shapFeatures = features.map((feature, index) => ({
    name: feature,
      ...
      <CardContent className="space-y-6">
          ...
          <div className="bg-gradient-to-r from-yellow-500/10 to-amber-500/10 border border-yellow-500/30 rounded-xl p-4">
            <div className="flex items-start gap-3">
              ...
              <div className="space-y-2">
                <h4 className="font-semibold text-yellow-400">Limited Explainability</h4>
                <p className="text-sm text-muted-foreground leading-relaxed">
                  Prediction confidence is limited due to insufficient UFC data or fighter debut status. 
                  The model defaults to a 50/50 prediction in these cases.
                </p>
              </div>
            </div>
          </div>
        ...
        {reason && (
          <div className="bg-background/30 rounded-xl p-4 border border-muted/20">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h5 className="font-medium text-blue-400 mb-1">Analysis Summary</h5>
            ...
            <div className="bg-gradient-to-r from-purple-500/10 to-blue-500/10 border border-purple-500/20 rounded-xl p-4">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-medium text-purple-400">Model Confidence</span>
                </div>
        ...
        {hasContribs && (
          <div className="space-y-4">
            <div className="flex items-center gap-2">
              ...
              <h5 className="font-semibold text-purple-400">Key Contributing Factors</h5>
            </div>
            <div className="grid gap-3">
              {topContributors.slice(0, isExpanded ? topContributors.length : 3).map((contrib, index) => (
                <div
                  key={index}
                  className={cn(
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className={cn(
                      ...
                      contrib.value > 0 ? "bg-green-500/30" : "bg-red-500/30"
                    )}>
                      {contrib.value > 0 ? (
                    ...
                    <div>
                      <span className="font-medium">{formatFeatureName(contrib.feature)}</span>
                      <div className="text-xs text-muted-foreground">
                        {contrib.value > 0 ? "Positive Impact" : "Negative Impact"}
                      </div>
                    </div>
                  </div>
                  <div className="text-right">
                    <span className={cn("font-bold text-lg", getImpactColor(contrib.value))}>
                      {contrib.value > 0 ? '+' : ''}{contrib.value.toFixed(3)}
                    </span>
                  </div>
                </div>
              ))}
        ...
        {isExpanded && hasShapData && (
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <h5 className="font-semibold text-purple-400">Detailed Feature Analysis</h5>
            </div>
              ...
              {shapFeatures.slice(0, 8).map((feature, index) => (
                <div key={index} className="space-y-2">
                  <div className="flex items-center justify-between text-sm">
                    <span className="font-medium">{formatFeatureName(feature.name)}</span>
                    <span className={cn("font-bold", getImpactColor(feature.value))}>
                    ...
                    <div className="w-full bg-muted/30 rounded-full h-2">
                      <div
                        ...
                        style={{
                          ...
                          marginLeft: feature.value < 0 ? `${100 - (feature.impact / maxImpact) * 100}%` : '0'
                        }}
                      />
                    </div>
                  </div>
            ...
            {expected_value !== undefined && (
              <div className="bg-background/30 rounded-lg p-4 border border-muted/20">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="font-medium text-blue-400">Model Baseline</span>
                </div>
                <p className="text-sm text-muted-foreground">
                  Expected baseline probability: <span className="font-bold">{(expected_value * 100).toFixed(1)}%</span>
                </p>
                <p className="text-xs text-muted-foreground mt-1">
                  SHAP values show how each feature pushes the prediction above or below this baseline.
                </p>
              </div>
            )}
          </div>
        ...
        {!hasContribs && !hasShapData && explainable && (
          <div className="text-center py-8 space-y-4">
            ...
            <div>
              <h4 className="font-semibold text-purple-400 mb-2">Analysis Complete</h4>
              <p className="text-sm text-muted-foreground">
                Model analysis available but no detailed feature contributions to display.
              </p>
            </div>
          </div>
        )}
